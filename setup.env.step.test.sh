...
[[ "$gh_pr" =~ ^[0-9]+$ ]] \
    && git_pr_msg="<$gh_url/pull/$gh_pr_num|$commit_msg>" \
    || git_pr_msg="$git_commit_msg"
success_fallback="$gha_url deployed successfully! $gh_pr_msg"
success_msg="${inputs_success_message}"
echo "SUCCESS_MESSAGE=${inputs_success_prefix} ${success_msg:-$success_fallback}" >> $GITHUB_ENV
failure_fallback=":boom: $gha_url failed! $gh_pr_msg"
